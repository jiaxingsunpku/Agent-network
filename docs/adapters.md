...
- `signalvision/`——**感知侧**（§2）+ **执行侧**（§3 信号控制，P6），同子包、职责分离、共用 client；
  当前已加 `WorldClient` 过渡双注册层：运行脚本保留老 traffic lifecycle，同时向世界级 lifecycle/heartbeat 自注册并声明 per-key 通道。
...
- **当前服务器过渡运行（2026-06-27）**：ANP `SignalVisionClient.start_simulation()` 调 SV
  `/api/simulation/start` 时显式带 `execution_mode="subprocess"`，避免 SV integration 模式在当前地图上秒退。
  SV Dashboard 已支持该单次启动模式，并修正子进程 cwd 到 `runtime/`、Python 解释器到当前 `traffic` env。
  若服务器未安装 SUMO，SV 会启动一个轻量演示回放子进程输出 `Step #n/total`，让
  `/api/simulation/status` 保持 `running=true` 且 `current_time` 持续递增；这用于演示环境保活，安装 SUMO 的真机仍走真实 SUMO。
